...
def DownloadHistoryTxnFile(FileID, fileseqno, transactionID ):
	...
	with open(requestfile, "r") as fin:
		...
		#print ("txnCSVRec : ", txnCSVRec)
		#nextFileNameloc = nextIsDownloadedloc
		#txnfileoutput.write(txnCSVRec + '\n')
		curlcommand = 'curl -X POST -H "Content-Type: text/xml; charset=utf-8" -H "SOAPAction: https://www.shafafiya.org/v2/DownloadTransactionFile" -d @'+requestfile+ ' -o ' +responsefile+ ' "https://shafafiya.doh.gov.ae/v3/webservices.asmx"'
		#print (curlcommand)
		logline = logwriter('i', 'Dnld:1.5 Processing Start : DownloadTransactionFile with the Curlcommand: '+ curlcommand)
		dlfh.write(f"{logline}")
		process = subprocess.Popen(curlcommand, shell=True, stdout=subprocess.PIPE, text=True)
		output, error = process.communicate()
		#print(output)
		logline = logwriter('i', 'Dnld:1.6 Output of the above Curlcommand : '+ output)
		dlfh.write(f"{logline}")
		with open(responsefile, "r") as infile, open(responsefile_1, "w") as outfile:
			...
			# v8g: removed no-op infile.close, outfile.close (missing parentheses) — with block handles closure
			...
			dlfh.write(f"{logline}")
	# v8g: removed no-op fin.close (missing parentheses) — with block handles closure
...
def build_and_execute_search_request(
	...
	logline = logwriter('i', f'His:Intv[{interval_idx}] Curl: {curlcommand}')
	dlfh.write(f"{logline}")

	process = subprocess.Popen(curlcommand, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
	output, error = process.communicate()

	if process.returncode != 0:
		logline = logwriter('w', f'His:Intv[{interval_idx}] Curl FAILED: {error.strip()}')
		dlfh.write(f"{logline}")
		return None

	# v8g: Check Shafafiya API result code in the response body.
	# A curl-successful response can still contain a Shafafiya error code e.g.:
	#   -3  invalid parameter
	#   -5  date range exceeds 100 days
	#  -10  no search criteria supplied
	# Previously these were silently passed to GetHistoryTxnFileDownload() which
	# would find no FileID patterns and exit with no warning in the log.
	try:
		with open(resp_fname, 'r', encoding='utf-8', errors='replace') as rf:
			resp_body = rf.read().replace('\n', ' ')
		sr_start = resp_body.find('<SearchTransactionsResult>')
		sr_end   = resp_body.find('</SearchTransactionsResult>')
		if sr_start > 0 and sr_end > sr_start:
			sr_code = resp_body[sr_start + 26 : sr_end].strip()
			if sr_code != '0':
				logline = logwriter('w', f'His:Intv[{interval_idx}] Shafafiya SearchTransactionsResult={sr_code} — interval skipped')
				dlfh.write(f"{logline}")
				return None
		# If tag not found the response may not contain a result code (e.g. empty/partial
		# response) — fall through and let GetHistoryTxnFileDownload handle gracefully.
	except OSError:
		pass   # If we cannot read the response file, fall through — curl already saved it

	logline = logwriter('i', f'His:Intv[{interval_idx}] Response saved: {resp_fname}')
	dlfh.write(f"{logline}")
	return resp_fname
...
def mainsub(downloadtype, active, claim_remit):
	...
	for idx, (wstart, wend) in enumerate(intervals):
		...
		if result:
			...
			dlfh.write(f"{logline}")
		# sleep(3) retained intentionally — curl is called via subprocess.Popen +
		# communicate() which is fully blocking, so this is NOT a correctness guard.
