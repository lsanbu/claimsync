...
#12. ClaimSync2 v2.12	Anbu			12-Mar-2026
#	Fix for [DISK FULL] 0.0 MB on Azure Files SMB mount.
#	Root cause: shutil.disk_usage() always returns 0 free bytes on an Azure Files
#	  SMB share mounted inside Container Apps — regardless of actual share quota.
#	  This is a known Azure platform limitation; the SMB client reports no free
#	  space information via statvfs().
#	Fix: When CLAIMSSYNC_BLOB_UPLOAD=1, set MIN_FREE_DISK_MB=0.
#	  Files are written to the mount then immediately deleted after blob upload,
#	  so there is no meaningful disk accumulation. The pre-write disk check
#	  condition (free_mb < MIN_FREE_DISK_MB) evaluates as (0.0 < 0) = False,
#	  so the check never fires and downloads proceed normally.
#	  When CLAIMSSYNC_BLOB_UPLOAD=0 (local mode), MIN_FREE_DISK_MB=50 is retained
#	  as a safety guard — no behaviour change for on-prem/local runs.
#	Changed:	main() — MIN_FREE_DISK_MB now blob-aware:
#	            0 when BLOB_UPLOAD=1, 50 when BLOB_UPLOAD=0
#	Fix 2: PermissionError on Windows path from DB config.
#	Root cause: tempfolder was loaded blindly from DB config which stores the
#	  on-prem Windows path (C:\Users\USER\Documents\MF2618\). Inside the Linux
#	  container this path is invalid — causing PermissionError on every write.
#	Fix: CLAIMSSYNC_TEMP_DIR env var now overrides DB config for tempfolder.
#	  DB path retained as fallback only when env var is absent.
#	Changed:	main() — tempfolder now env-var aware (CLAIMSSYNC_TEMP_DIR wins over DB)
...
#	            updated HisDnld:1.5 log message, HisDnld:1.6 now genuine-failure-only
#14. ClaimSync2 v3.3	Anbu			13-Mar-2026
#	Phase 3 — P3-T02: DB run logging (sync_run_log + sync_run_intervals + file_manifest).
#	New module: db_run_logger.py — DBRunLogger class (non-fatal, write-only).
#	Gate: CLAIMSSYNC_DB_LOGGING=1 → ACTIVE; absent/0 → disabled silently.
#	New globals: _db_logger, _current_run_id, _interval_id_map, _run_stats, _current_interval_id
#	Changed: main() — DBRunLogger init after blob init (Pre:DB)
#	Changed: main() facility loop — per-facility _run_stats reset + start_run + end_run
#	Changed: main() OSError handler — end_run(status='failed')
#	Changed: mainsub() — global declaration + start_run on claims pass (h only)
#	Changed: mainsub() interval loop — log_interval after each API call
#	Changed: GetHistoryTxnFileDownload() — global declaration + log_file for XML + ZIP_EXTRACTED
#	Bug fix (v3.2): Added global declarations for _current_run_id etc in mainsub()
#	  and GetHistoryTxnFileDownload() — fixes UnboundLocalError from Python scoping.
#13. ClaimSync2 v2.8	Anbu			12-Mar-2026
#	Phase 3 — P3-T01: Azure Blob Storage upload activation.
#	Implements _blob_upload_file() which was a stub in v2.4–v2.7.
#	Gate:  CLAIMSSYNC_BLOB_UPLOAD=0 → no-op (all prior behaviour preserved).
#	       CLAIMSSYNC_BLOB_UPLOAD=1 → uploads each downloaded file to
#	         container claimssync-{facility.lower()} using Managed Identity.
...
#	New module: blob_storage_provider.py — init_blob_client() + upload_file()
#	            uses DefaultAzureCredential (Managed Identity, no keys/secrets)
#	New env var: CLAIMSSYNC_STORAGE_URL (default: https://stclaimssyncuae.blob.core.windows.net)
#	Changed:	imports — added _blob_service_client, _blob_enabled globals
#	Added:		_init_blob_client() — lazy import, called once in main() if enabled
#	Added:		_blob_upload_file() — implements the Phase 2 stub at line 994
#	Changed:	main() globals — added _blob_enabled, _blob_service_client, _blob_ct
#	Changed:	main() — blob init block after config load (Pre:1.5b / Pre:1.5c)
#	Changed:	main() facility loop — _blob_ct set per-facility (Pre:1.10b)
...
# when azure-storage-blob is absent (e.g. local dev without the package).
# _blob_service_client  — BlobServiceClient instance (None if BLOB_UPLOAD=0)
...
# All DB failures are non-fatal — logged as WARNING, run continues.
_db_logger           = None   # DBRunLogger instance
_current_run_id      = None   # UUID str — active sync_run_log row for current facility
_interval_id_map     = {}     # {interval_idx: interval_id UUID str}
_run_stats           = {}     # accumulated counters written to sync_run_log via end_run()
_current_interval_id = None   # resolved per response file in hff loop


def _init_blob_client(account_url: str):
    """Initialise BlobServiceClient via Managed Identity. Called once in main()."""
    global _blob_service_client
    from blob_storage_provider import init_blob_client
    _blob_service_client = init_blob_client(account_url)


def _blob_upload_file(local_path: str, container_name: str, ftype: str, facility: str):
    """
    Upload a downloaded file to Azure Blob Storage.
    No-op (returns immediately) when CLAIMSSYNC_BLOB_UPLOAD != 1.

    Blob path: {ftype}/{os.path.basename(local_path)}
               e.g. claims/MF2618_H13238_OP_NEURON_110326_5_1.xml
                    remittance/351.24-24200898-NRML-11032026-11-20-00.zip
    """
    if not _blob_enabled or _blob_service_client is None:
        return  # BLOB_UPLOAD=0 — silently skip

    blob_name = f"{ftype}/{os.path.basename(local_path)}"
    logline = ''
    try:
        from blob_storage_provider import upload_file as _do_upload
        ok = _do_upload(_blob_service_client, container_name, blob_name, local_path)
        if ok:
            print(f'BlobUpload:OK  {container_name}/{blob_name}')
            logline = logwriter('i', f'BlobUpload:OK  {container_name}/{blob_name}')
            # Delete local file after successful upload — keep Azure Files scratch clean
            try:
                os.remove(local_path)
                print(f'BlobUpload:LOCAL_DEL {local_path}')
            except Exception:
                pass
        else:
            print(f'BlobUpload:FAIL {container_name}/{blob_name}')
            logline = logwriter('w', f'BlobUpload:FAIL {container_name}/{blob_name} — see blob_storage_provider log')
    except Exception as exc:
        print(f'BlobUpload:ERR {blob_name}: {exc}')
        logline = logwriter('w', f'BlobUpload:ERR {blob_name}: {exc}')
    if logline:
        dlfh.write(f"{logline}")
# ── v3.8: Audit blob upload (no delete) ──────────────────────────────────────
def _blob_upload_audit(local_path: str, container_name: str, blob_prefix: str):
    """Upload a file to blob WITHOUT deleting the local copy. Used for audit files
    (search_history XML, log files, CSV manifests) that must persist locally for
...
_facility_csv_path = None    # set per-facility in main()
_facility_csv_rows = []      # accumulated CSV rows


def _facility_log(msg: str):
    """Write a line to the per-facility log file (if open)."""
    global _facility_log_fh
    if _facility_log_fh:
        now = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        _facility_log_fh.write(f"INFO : {now} : {msg}\n")


def _facility_csv_add(row: dict):
    """Accumulate a row for the per-facility CSV manifest."""
    global _facility_csv_rows
    _facility_csv_rows.append(row)
...
def GetHistoryTxnFileDownload(xmlBody, transactionID):
	global _db_logger, _current_run_id, _run_stats, _facility_csv_rows
	...
	downloadedfileslog = os.path.join(tempfolder, 'downloadlog-'+facility+'-'+formatted_datetime+'.csv')
	...
	with open(downloadedfileslog, 'w') as txnfileoutput:
		...
		while nextFileNameloc > 0:
			...
			txnCSVRecHeader = FileID + "," + FileName + "," + SenderID + "," + ReceiverID + "," + TransactionDate + "," + RecordCount + "," + TransactionTimestamp + "," + IsDownloaded
			txnfileoutput.write(txnCSVRecHeader + '\n')
			# v3.8: accumulate for per-facility CSV + pass metadata to log_file
			_facility_csv_add({
				'FileID': FileID, 'FileName': FileName, 'SenderID': SenderID,
				'ReceiverID': ReceiverID, 'TransactionDate': TransactionDate,
				'RecordCount': RecordCount, 'TransactionTimestamp': TransactionTimestamp,
				'IsDownloaded': IsDownloaded,
			})
			# Store last-parsed row metadata for log_file enrichment
			_last_file_meta = {
				'shafafiya_txn_id': FileID, 'sender_id': SenderID,
				'receiver_id': ReceiverID, 'record_count': RecordCount,
				'transaction_date': TransactionDate, 'transaction_timestamp': TransactionTimestamp,
			}
			...
			if downloadedFilename == '':
				...
				dlfh.write(f"{logline}")
				if _run_stats:
					_run_stats['files_downloaded'] = _run_stats.get('files_downloaded', 0) + 1
					if transactionID == '8':
						_run_stats['files_remittance'] = _run_stats.get('files_remittance', 0) + 1
				# Phase 3: upload all files extracted from this ZIP to blob storage
				if _blob_enabled:
					_ftype = 'claims' if transactionID == '2' else 'remittance'
					_dest_folder = claims if transactionID == '2' else remittance
						...
						if os.path.isfile(_extracted_path):
							...
							if _db_logger and _current_run_id:
								try:
									...
									logline = logwriter('w', f'DB:log_file failed (non-fatal): {_exc}')
									dlfh.write(f"{logline}")
			elif os.path.exists(downloadedFilename):
				count_downloaded += 1
				print(f'  [XML-OK] {os.path.basename(downloadedFilename)} exists on disk, uploading...')
				...
				if _db_logger and _current_run_id:
					try:
						...
						logline = logwriter('w', f'DB:log_file failed (non-fatal): {_exc}')
						dlfh.write(f"{logline}")
			elif transactionID == '2' and os.path.exists(os.path.join(resubmission, os.path.basename(downloadedFilename))):
				# v3.9: File was moved to resubmission/ by FindnMoveResubmisison — still a success.
				# Blob upload + log_file for resubmission files is handled in the post-batch block.
				count_downloaded += 1
				logline = logwriter('i', f'HisDnld:1.5r FileID {FileID} moved to resubmission/ (contains <Resubmission> tag)')
				dlfh.write(f"{logline}")
				if _run_stats:
					_run_stats['files_downloaded'] = _run_stats.get('files_downloaded', 0) + 1
				_facility_log(f'Dwn:{os.path.basename(downloadedFilename)} -> resubmission/')
			else:
				count_notfound += 1
	...
	if transactionID == '2':
		...
		# v3.9: Resubmission blob upload moved to main() finally: block
		# to run ONCE after ALL GetHistoryTxnFileDownload batches complete.
...
def build_and_execute_search_request(
	...
	result = soap_search_transactions(req_fname, resp_fname, interval_idx, logwriter, dlfh)

	# ── v3.8: Upload search_history XMLs to blob (audit — no delete) ──────
	_facility_log(f'His:Intv[{interval_idx}] Request: {req_fname} | {interval_from} to {interval_to}')
	if result:
...
def mainsub(downloadtype, active, claim_remit):
	...
	#Option 3: downloadTxnFilesv7 hf
		...
		_batfile = os.path.join(tempfolder, "downloadhistfileids.bat")
		with open(_batfile, "r") as batfile:
	...
	# ── v3.6: Adhoc date range override via env vars ────────────────────
	_adhoc_from = os.environ.get('CLAIMSSYNC_ADHOC_FROM', '').strip()
	_adhoc_to   = os.environ.get('CLAIMSSYNC_ADHOC_TO',   '').strip()

	if _adhoc_from and _adhoc_to:
		# Adhoc run — parse YYYY-MM-DD or YYYY-MM-DD HH:MM into Shafafiya format
		def _parse_adhoc_dt(val):
			"""Parse 'YYYY-MM-DD' or 'YYYY-MM-DD HH:MM' → Shafafiya DD/MM/YYYY HH:MM:SS"""
			for fmt in ('%Y-%m-%d %H:%M', '%Y-%m-%d'):
				try:
					dt = datetime.strptime(val, fmt)
					return dt.strftime(SHAFAFIYA_DATE_FMT)
				except ValueError:
					continue
			return None

		transactionFromDate = _parse_adhoc_dt(_adhoc_from)
		transactionToDate   = _parse_adhoc_dt(_adhoc_to)
		if not transactionFromDate or not transactionToDate:
			logline = logwriter('e', f'His:ADHOC Date parse error: from={_adhoc_from} to={_adhoc_to}')
			dlfh.write(f"{logline}")
			return
		logline = logwriter('i', f'His:ADHOC ADHOC RUN: facility={facility} from={transactionFromDate} to={transactionToDate}')
		dlfh.write(f"{logline}")
		print(f'  [ADHOC RUN] facility={facility} from={transactionFromDate} to={transactionToDate}')
	elif defaultsearch.lower() == 'y':
	...
	if _run_stats:
		_run_stats['intervals_total'] = _run_stats.get('intervals_total', 0) + total_intervals
	...
	for idx, (wstart, wend) in enumerate(intervals):
		...
		if result:
			successful += 1
			if _run_stats:
				_run_stats['intervals_completed'] = _run_stats.get('intervals_completed', 0) + 1
		else:
			failed += 1
			if _run_stats:
...
def main():
    ...
    # deleted after upload). Azure Files SMB mounts also report 0 free bytes via
    # shutil.disk_usage() regardless of actual quota — a known platform limitation.
    # Setting to 0 disables the pre-write disk check in blob mode entirely.
    MIN_FREE_DISK_MB = 0 if os.environ.get('CLAIMSSYNC_BLOB_UPLOAD', '0').strip() == '1' else 50
    ...
    # ── Phase 3: Blob Storage init ─────────────────────────────────────────
    _blob_enabled = os.environ.get('CLAIMSSYNC_BLOB_UPLOAD', '0').strip() == '1'
    _blob_ct      = None   # set per-facility in the loop below
    if _blob_enabled:
        ...
        dlfh.write(f"{logline}")
        try:
            _init_blob_client(_storage_url)
            logline = logwriter('i', 'Pre:1.5c BlobServiceClient initialised OK (Managed Identity)')
            dlfh.write(f"{logline}")
        except Exception as exc:
            print(f"\033[31m[ClaimSync2] FATAL: Blob init failed — {exc}\033[0m")
            logline = logwriter('c', f'Crit:1.3 Blob init failed: {exc}')
            dlfh.write(f"{logline}")
            dlfh.close()
            return
    else:
        print("[ClaimSync2] Blob upload DISABLED (CLAIMSSYNC_BLOB_UPLOAD=0) — files stay local")
        logline = logwriter('i', 'Pre:1.5b Blob upload disabled (CLAIMSSYNC_BLOB_UPLOAD=0)')
        dlfh.write(f"{logline}")

    # ── P3-T02: DB Run Logger init ─────────────────────────────────────────
    _db_logging_enabled = os.environ.get('CLAIMSSYNC_DB_LOGGING', '0') == '1'
    if _db_logging_enabled:
        try:
            from db_run_logger import DBRunLogger
            _db_logger = DBRunLogger()
            _db_logger.connect()
            logline = logwriter('i', 'Pre:DB DBRunLogger connected — DB logging ACTIVE')
            dlfh.write(f"{logline}")
            print('[ClaimSync2] DB logging: ACTIVE')
        except Exception as _db_exc:
            logline = logwriter('w', f'Pre:DB DBRunLogger init failed (non-fatal — run continues): {_db_exc}')
            dlfh.write(f"{logline}")
            print(f'[ClaimSync2] DB logging: DISABLED (init error: {_db_exc})')
            _db_logger = None
    else:
        print('[ClaimSync2] DB logging: DISABLED (CLAIMSSYNC_DB_LOGGING != 1)')

    # ── Facility loop setup ────────────────────────────────────────────────
    noofsetup    = int(config['shafaapi-main']['noofsetup'])
    # Cloud override: CLAIMSSYNC_TEMP_DIR env var takes precedence over DB config.
    # DB config stores the on-prem Windows path (e.g. C:\Users\USER\Documents\MF2618\)
    # which is invalid inside the Linux container. The env var (/mnt/claimssync/ or
    # /tmp/claimssync/) must always win in cloud deployments.
    _db_tempfolder = config['shafaapi-main']['tempfolder'].strip('"')
    tempfolder     = os.environ.get('CLAIMSSYNC_TEMP_DIR', _db_tempfolder).rstrip('/\\') + '/'
    systemfolder   = tempfolder  # cloud: no separate systemfolder — same as tempfolder

    logline = logwriter('i', f'Pre:1.6 noofsetup={noofsetup} tempfolder={tempfolder} (db_path={_db_tempfolder}) systemfolder={systemfolder}')
    dlfh.write(f"{logline}")

    # ── Clean temp folder (v3.5: purge ALL stale XML + CSV) ──────────────
    try:
        os.makedirs(tempfolder, exist_ok=True)
        purged = 0
        for file in os.listdir(tempfolder):
            fname = Path(file)
            if fname.suffix.lower() == ".xml":
                os.remove(os.path.join(tempfolder, file))
                purged += 1
            elif fname.suffix.lower() == ".csv":
                os.remove(os.path.join(tempfolder, file))
                purged += 1
        if purged:
            logline = logwriter('i', f'Pre:1.7a Purged {purged} stale XML/CSV file(s) from temp folder')
            dlfh.write(f"{logline}")
    except Exception as exc:
        logline = logwriter('w', f'Pre:1.7b temp folder cleanup warning: {exc}')
        dlfh.write(f"{logline}")

    # ── v3.6: Adhoc facility filter ─────────────────────────────────────
    _adhoc_facility = os.environ.get('CLAIMSSYNC_ADHOC_FACILITY', '').strip().upper()
    _adhoc_from_env = os.environ.get('CLAIMSSYNC_ADHOC_FROM', '').strip()
    _adhoc_to_env   = os.environ.get('CLAIMSSYNC_ADHOC_TO',   '').strip()
    if _adhoc_from_env and _adhoc_to_env:
        print(f'[ClaimSync2] ADHOC RUN: from={_adhoc_from_env} to={_adhoc_to_env}'
              f'{" facility=" + _adhoc_facility if _adhoc_facility else " (all facilities)"}')
        logline = logwriter('i', f'Pre:ADHOC ADHOC RUN mode: from={_adhoc_from_env} to={_adhoc_to_env}'
                                 f' facility_filter={_adhoc_facility or "(all)"}')
        dlfh.write(f"{logline}")
    ...
    try:
        for currentsetup in range(1, noofsetup + 1):
            ...
            # v3.6: skip facilities not matching adhoc filter
            if _adhoc_facility and facility.upper() != _adhoc_facility:
                print(f'[ClaimSync2] ADHOC: skipping facility {facility} (filter={_adhoc_facility})')
                logline = logwriter('i', f'Pre:ADHOC Skipping facility {facility} — adhoc filter={_adhoc_facility}')
                dlfh.write(f"{logline}")
                continue
            # Cloud override: claims/remittance/resubmission paths from DB are Windows
            # paths (e.g. C:\Users\USER\Documents\MF2618\claims) — invalid in Linux.
            # When CLAIMSSYNC_TEMP_DIR is set, rebuild all subfolder paths under tempfolder.
            _db_claims       = config[section]['claims']
            _db_resubmission = config[section]['resubmission']
            _db_remittance   = config[section]['remittance']
            if os.environ.get('CLAIMSSYNC_TEMP_DIR'):
                claims       = os.path.join(tempfolder, 'claims') + '/'
                resubmission = os.path.join(tempfolder, 'resubmission') + '/'
                remittance   = os.path.join(tempfolder, 'remittance') + '/'
                os.makedirs(claims,       exist_ok=True)
                os.makedirs(resubmission, exist_ok=True)
                os.makedirs(remittance,   exist_ok=True)
            else:
                claims       = _db_claims
                resubmission = _db_resubmission
                remittance   = _db_remittance

            logline = logwriter('i', f'Pre:1.10 Facility {facility} config loaded from DB+KV')
            dlfh.write(f"{logline}")

            # Phase 3: container name follows convention claimssync-{facility.lower()}
            _blob_ct = f"claimssync-{facility.lower()}"
            if _blob_enabled:
                logline = logwriter('i', f'Pre:1.10b Blob container for this facility: {_blob_ct}')
                dlfh.write(f"{logline}")

            # ── P3-T02: reset per-facility DB run state ───────────────────
            _current_run_id      = None
            _interval_id_map     = {}
            _current_interval_id = None
            _run_stats = dict(
                intervals_total=0, intervals_completed=0, intervals_skipped=0,
                files_found=0, files_downloaded=0, files_skipped_api_error=0,
                files_remittance=0, files_resubmission=0,
            )

            # ── v3.8: Per-facility log + CSV init ─────────────────────────
            _fac_ts = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
            _facility_log_path = os.path.join(log_dir, f'downloadlog-{facility}-{_fac_ts}.log')
            _facility_log_fh   = open(_facility_log_path, 'a')
            _facility_csv_path = os.path.join(log_dir, f'downloadlog-{facility}-{_fac_ts}.csv')
            _facility_csv_rows = []
            _facility_log(f'Pre:1.10 Facility {facility} run started')

            # ── v3.7: Compute date range + trigger_type BEFORE mainsub ────
            # This ensures start_run always has valid from/to dates regardless
            # of what happens inside mainsub.
            SHAFAFIYA_DATE_FMT = '%d/%m/%Y %H:%M:%S'
            _is_adhoc = bool(_adhoc_from_env and _adhoc_to_env)
            _trigger_type = 'manual' if _is_adhoc else 'scheduled'
            if _is_adhoc:
                # Parse adhoc dates for the run record
                def _parse_adhoc_for_run(val):
                    for fmt in ('%Y-%m-%d %H:%M', '%Y-%m-%d'):
                        try:
                            return datetime.strptime(val, fmt).strftime(SHAFAFIYA_DATE_FMT)
                        except ValueError:
                            continue
                    return None
                _run_from_date = _parse_adhoc_for_run(_adhoc_from_env) or _adhoc_from_env
                _run_to_date   = _parse_adhoc_for_run(_adhoc_to_env) or _adhoc_to_env
            else:
                # Scheduled: yesterday 00:00 → now (same logic as mainsub default)
            ...
            if _db_logger:
                try:
                    _current_run_id = _db_logger.start_run(
                        facility_code=facility,
                    ...
                    if _current_run_id:
                        logline = logwriter('i', f'DB:start_run run_id={_current_run_id} '
                                                 f'trigger={_trigger_type} {_run_from_date}→{_run_to_date}')
                        dlfh.write(f"{logline}")
                        print(f'DB:start_run run_id={_current_run_id} trigger={_trigger_type}')
                except Exception as _exc:
                    logline = logwriter('w', f'DB:start_run failed (non-fatal): {_exc}')
                    dlfh.write(f"{logline}")
            ...
            try:
                ...
                # ── v3.9: Upload resubmission files to blob ───────────────
                # Runs ONCE after ALL claims+remittance batches complete.
                # By this point FindnMoveResubmisison has moved files to
                # resubmission/ and remove_attachments has stripped tags.
                if _blob_enabled and os.path.isdir(resubmission):
                    _resub_files = [f for f in os.listdir(resubmission) if os.path.isfile(os.path.join(resubmission, f))]
                    if _resub_files:
                        ...
                        for _rf in _resub_files:
                            ...
                            if _run_stats:
                                _run_stats['files_resubmission'] = _run_stats.get('files_resubmission', 0) + 1
                            if _db_logger and _current_run_id:
                                try:
                                    _db_logger.log_file(
                                        facility_code=facility,
                ...
                dlfh.write(f"{logline}")
                print(f'\033[31m[ClaimSync2] Facility {facility} error: {_fac_exc}\033[0m')

            finally:
                # ── v3.8: Finalize per-facility log + CSV ─────────────────
                _facility_log('BAU run complete')
                if _facility_log_fh:
                    _facility_log_fh.close()
                    _facility_log_fh = None
                # Write CSV
                if _facility_csv_path and _facility_csv_rows:
                    try:
                        with open(_facility_csv_path, 'w') as _csvf:
                            _csvf.write('FileID,FileName,SenderID,ReceiverID,TransactionDate,RecordCount,TransactionTimestamp,IsDownloaded\n')
                            for _r in _facility_csv_rows:
                                _csvf.write(f'{_r.get("FileID","")},{_r.get("FileName","")},{_r.get("SenderID","")},{_r.get("ReceiverID","")},{_r.get("TransactionDate","")},{_r.get("RecordCount","")},{_r.get("TransactionTimestamp","")},{_r.get("IsDownloaded","Y")}\n')
                        print(f'[ClaimSync2] CSV manifest written: {_facility_csv_path} ({len(_facility_csv_rows)} rows)')
                    except Exception as _csv_exc:
                        print(f'[ClaimSync2] CSV write error (non-fatal): {_csv_exc}')
                elif _facility_csv_path:
                ...
                if _db_logger and _current_run_id:
                    ...
                    try:
                        _db_logger.end_run(
                            ...
                            error_message=_facility_error,
                            **_run_stats,
                        )
                        logline = logwriter('i', f'DB:end_run run_id={_current_run_id} status={_final_status} '
                                                f'files_dl={_run_stats.get("files_downloaded",0)} '
                                                f'intervals={_run_stats.get("intervals_completed",0)}/'
                                                f'{_run_stats.get("intervals_total",0)}')
                        dlfh.write(f"{logline}")
                        print(f'[ClaimSync2] DB:end_run run_id={_current_run_id} status={_final_status} '
                              f'files_dl={_run_stats.get("files_downloaded",0)} '
                              f'intervals={_run_stats.get("intervals_completed",0)}/'
                              f'{_run_stats.get("intervals_total",0)}')
                    except Exception as _exc:
        ...
        if ose.errno == errno.ENOSPC:
            ...
            if _db_logger and _current_run_id:
                try:
                    _db_logger.end_run(run_id=_current_run_id, status='failed',
                                       error_message='OSError errno 28 — No space left on device', **_run_stats)
                except Exception:
                    pass
        else:
