...
#13. ClaimSync2 v3.1	Anbu			13-Mar-2026
#14. ClaimSync2 v3.2	Anbu			13-Mar-2026
#	Bug fix: Added `global _db_logger, _current_run_id, ...` declarations to
#	mainsub() and GetHistoryTxnFileDownload() — fixes UnboundLocalError on
#	_current_run_id when Python treats it as local due to assignment in scope.
#	Phase 3 P3-T02 — DB run logging to sync_run_log, sync_run_intervals,
#	file_manifest via new db_run_logger.py module.
#	Gated by CLAIMSSYNC_DB_LOGGING=1 env var — zero cost when disabled.
#	All DB errors are non-fatal: a DB failure never aborts a sync run.
#	New module:	db_run_logger.py — DBRunLogger class
#	           Supports CLAIMSSYNC_DB_DSN (DSN/password env var) and
#	           DefaultAzureCredential AAD token (Managed Identity) auth.
#	New globals: _db_logger, _current_run_id, _interval_id_map,
#	             _run_stats, _current_interval_id
#	Changed:	main() — DBRunLogger init after config load (Pre:DB block)
#	           Added global declarations for DB state
#	           Per-facility: _run_stats reset + DB start_run + DB end_run
#	           OSError disk-full handler: end_run with status='failed'
#	Changed:	mainsub() h handler — DB start_run on claims pass (first h call)
#	           Interval loop: DB log_interval after each API call,
#	           _run_stats.intervals_* incremented here
#	Changed:	mainsub() hff handler — _current_interval_id resolved from
#	           filename index before each GetHistoryTxnFileDownload call
#	Changed:	GetHistoryTxnFileDownload() — DB log_file for each confirmed
#	           download (both XML and ZIP_EXTRACTED paths),
#	           _run_stats.files_* incremented here
	...
	with open(downloadedfileslog, 'w') as txnfileoutput:
		...
		while nextFileNameloc > 0:
			...
			if downloadedFilename == '':
				...
				dlfh.write(f"{logline}")
				_run_stats['files_skipped_api_error'] += 1
				...
				if _db_logger and _current_run_id:
					try:
						_db_logger.log_file(
							...
							file_name=FileName,
							file_type=_ftype,
							shafafiya_txn_id=FileID,
							interval_id=_current_interval_id,
						)
						_run_stats['files_downloaded'] += 1
						if _ftype == 'remittance':
							_run_stats['files_remittance'] += 1
				...
				if _db_logger and _current_run_id:
					try:
						_fsize = None
						try:
							_fsize = os.path.getsize(downloadedFilename)
						except OSError:
							pass
						_db_logger.log_file(
							facility_code=facility,
...
def mainsub(downloadtype, active, claim_remit):
	...
	#Option 4: downloadTxnFilesv7 hff
		...
		for resp_file in response_files:
			...
			dlfh.write(f"{logline}")
			# ── P3-T02: resolve interval_id for file_manifest linkage ─────
			_current_interval_id = None
			if _db_logger and _current_run_id:
				import re as _re_hff
				_m = _re_hff.search(r'_(\d+)\.xml$', resp_file)
				if _m:
					_current_interval_id = _interval_id_map.get(int(_m.group(1)))
	...
	# ── P3-T02: start DB run on the claims pass (first h call per facility) ──
	if claim_remit == 'claim' and _db_logger and _current_run_id is None:
		try:
			_sfrom = datetime.strptime(transactionFromDate.strip(), '%d/%m/%Y %H:%M:%S').date()
			_sto   = datetime.strptime(transactionToDate.strip(),   '%d/%m/%Y %H:%M:%S').date()
			...
			if _current_run_id:
				logline = logwriter('i', f'DB:start_run run_id={_current_run_id} '
				                        f'{transactionFromDate} → {transactionToDate}')
				dlfh.write(f"{logline}")
		except Exception as _exc:
	...
	for idx, (wstart, wend) in enumerate(intervals):
		...
		if result:
			...
			dlfh.write(f"{logline}")
		# ── P3-T02: log interval result to DB ─────────────────────────────
		if _db_logger and _current_run_id:
			try:
				_int_id = _db_logger.log_interval(
					run_id=_current_run_id,
					interval_index=idx,
					interval_from=wstart,
					interval_to=wend,
					api_result_code='0' if result else 'ERR',
					files_in_response=0,   # populated in hff phase; 0 at search time
					status='success' if result else 'error',
				)
				_interval_id_map[idx] = _int_id   # None on DB error — hff will handle gracefully
			except Exception as _exc:
				logline = logwriter('w', f'DB:log_interval failed (non-fatal): {_exc}')
				dlfh.write(f"{logline}")
		_run_stats['intervals_total'] += 1
		if result:
			_run_stats['intervals_completed'] += 1
		else:
			_run_stats['intervals_skipped'] += 1
...
def main():
    ...
    # P3-T02: DB run logging state — shared across main(), mainsub(), GetHistoryTxnFileDownload()
    global _db_logger, _current_run_id, _interval_id_map, _run_stats, _current_interval_id
    MIN_FREE_DISK_MB = 50
    _db_logger         = None   # DBRunLogger instance (None = DB logging disabled)
    ...
    _run_stats         = {}     # accumulated counters for end_run()
    ...
    try:
        for currentsetup in range(1, noofsetup + 1):
            ...
            _run_stats = dict(
                ...
                files_resubmission=0, files_remittance=0,
            )
            ...
            # ── P3-T02: close the run for this facility ───────────────────
            if _db_logger and _current_run_id:
                try:
                    _db_logger.end_run(
                        run_id=_current_run_id,
                    ...
                    logline = logwriter('i', f'DB:end_run run_id={_current_run_id} status=success '
                                            f'files_dl={_run_stats["files_downloaded"]} '
                                            f'intervals={_run_stats["intervals_completed"]}/{_run_stats["intervals_total"]}')
                    dlfh.write(f"{logline}")
                except Exception as _exc:
        ...
        if ose.errno == errno.ENOSPC:
            ...
            # ── P3-T02: mark the current run as failed due to disk full ───
            if _db_logger and _current_run_id:
                try:
                    _db_logger.end_run(
                        run_id=_current_run_id,
