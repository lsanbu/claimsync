...
def GetHistoryTxnFileDownload(xmlBody, transactionID):
	global _db_logger, _current_run_id, _run_stats
...
def mainsub(downloadtype, active, claim_remit):
	...
	# ── P3-T02: start_run once per facility (claims h pass only) ──────────
	if claim_remit == 'claim' and _db_logger and _current_run_id is None:
		try:
			_current_run_id = _db_logger.start_run(
				facility_code=facility,
				search_from=transactionFromDate,
				search_to=transactionToDate,
			)
			if _current_run_id:
				logline = logwriter('i', f'DB:start_run run_id={_current_run_id} {transactionFromDate}→{transactionToDate}')
				dlfh.write(f"{logline}")
				print(f'DB:start_run run_id={_current_run_id}')
		except Exception as _exc:
        ...
        for currentsetup in range(1, noofsetup + 1):
            ...
            if _db_logger and _current_run_id:
                try:
                    ...
                    logline = logwriter('i', f'DB:end_run run_id={_current_run_id} status=success '
                    ...
                    dlfh.write(f"{logline}")
                    print(f'[ClaimSync2] DB:end_run run_id={_current_run_id} status=success '
                          f'files_dl={_run_stats.get("files_downloaded",0)} '
                          f'intervals={_run_stats.get("intervals_completed",0)}/'
                          f'{_run_stats.get("intervals_total",0)}')
                except Exception as _exc:
