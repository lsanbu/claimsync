...
#11. downloadTxnFilesv8g	Anbu			10-Mar-2026
#	Code quality and correctness fixes identified in pre-SaaS code review.
#	No functional behaviour changed for normal successful runs.
#	(a) SOAP closing tag typo — double >> at end of </v2:DownloadTransactionFile>>
#	    Present since early versions. Shafafiya server was tolerant but any server-side
#	    XML parser tightening could start rejecting the malformed request. Fixed to
#	    single > as per XML spec.
#	(b) No-op .close calls — hrf.close, infile.close, outfile.close, fin.close,
#	    txnfileoutput.close all referenced the method object without calling it (missing
#	    parentheses). Files were correctly closed by their 'with' block context managers
#	    so no data loss occurred, but the dead code was misleading. Removed all no-op
#	    references. The two genuine hrf.close() calls with parentheses are retained.
#	(c) SearchTransactions Shafafiya result code not checked — build_and_execute_search_
#	    request() checked curl process.returncode (OS-level exit code) but never read the
#	    SOAP response body for Shafafiya API result codes. A curl-successful response
#	    containing SearchTransactionsResult=-3 (invalid param), -5 (>100 day range),
#	    or -10 (no search criteria) was silently passed to GetHistoryTxnFileDownload()
#	    which would find no FileID patterns and exit silently — no warning in log.
#	    Fix: after curl succeeds, read response file and check for
#	    <SearchTransactionsResult> tag. If value != '0', log WARNING with the result
#	    code and return None (same as curl failure path) so the interval is skipped
#	    and counted as failed in the interval summary.
#	(d) maxRecordCount=100000 in INI createini() — API spec §2.2.4 caps at 1000
#	    regardless of requested value. Corrected default to 1000 and added comment.
#	(e) transactionStatus=2 in INI createini() — value 2 = "already downloaded"
#	    which is correct for historical backfill. For daily scheduled sync the correct
#	    value is 1 (new/undownloaded only). Added clarifying comment in createini().
#	Note: Hardcoded TOTP secret in askitsolsauthenticate() (line ~331) is a known
#	    security issue deferred to SaaS migration — changing it would invalidate
#	    Saleem's existing authenticator app setup on the live deployment.
#	Changed:	FormatDownloadTxnRequestFile() line008 — removed extra > from closing tag
#	Changed:	Multiple locations — removed 6 no-op .close references (no parentheses)
#	Changed:	build_and_execute_search_request() — added Shafafiya result code check
#	            on response body after curl success
#	Changed:	createini() — maxRecordCount default 100000 → 1000 + comment added
#	Changed:	createini() — transactionStatus=2 comment clarified (backfill vs daily)
#12. downloadTxnFilesv8g (fix)	Anbu			11-Mar-2026
#	Fix for hff step downloading files from previous runs (wrong-date files).
#	Root cause (pre-existing since v8b):
#	  The h step writes search_history_response_{facility}_{claim_remit}_{idx}.xml
#	  files numbered 0..N-1 for the current run's interval count.
#	  Previous runs with a wider date range (more intervals) left higher-numbered
#	  response files in the Temp folder. The hff glob pattern _*.xml picks up ALL
#	  files matching the facility+claim_remit prefix regardless of run date —
#	  so old-date FileIDs were re-downloaded alongside the current day's files.
#	Fix:
#	  At the start of the h handler (after date range is set, before interval loop),
#	  purge all existing search_history_response_{facility}_{claim_remit}_*.xml
#	  and search_history_request_{facility}_{claim_remit}_*.xml files from systemfolder.
#	  hff now only sees response files written in the current run.
#	Changed:	mainsub() h handler — added pre-loop purge of stale interval XML files
#
# ============================================================================
# ClaimSync1a.py	Anbu	11-Mar-2026
# ============================================================================
# SaaS baseline — first version under the ClaimSync product name.
# Renamed from downloadTxnFilesv8g_fix2.py. Incorporates all fixes through
# v8g_fix2 plus Phase 0 Foundation Hardening changes:
#
# Phase 0 changes applied on top of v8g_fix2:
#   (a) httpx replaces curl subprocess — soap_search_transactions() and
#       soap_download_transaction() from httpx_soap.py replace all
#       subprocess.Popen/curlcommand blocks. Synchronous/blocking, no shell=True,
#       explicit 60s connect / 120s read timeouts (curl had none).
#   (b) ConfigProvider abstraction — LocalINIProvider from config_provider.py
#       replaces raw configparser.ConfigParser() + config.read() in main().
#       All config['section']['key'] accesses unchanged. Phase 2 swap:
#       provider = DBConfigProvider(tenant_id) — zero other changes needed.
#   (c) sleep(10) removed — 4 × sleep(10) between mainsub() calls in facility
#       loop removed. httpx is synchronous/blocking; the sleeps were a curl
#       subprocess timing workaround, now redundant.
#       sleep(3) between intervals retained — courtesy rate-limit to Shafafiya.
#
# Complete fix lineage in this file:
#   v8b  — 2-hr interval windowing (silent 1000-file API cap)
#   v8c  — DownloadTransactionFileResult check before parsing
#   v8d  — Disk-full OSError handling
#   v8e  — O(n²) → O(n) resubmission attachment scan
#   v8f  — HisDnld:1.6 false positive / ZIP_EXTRACTED sentinel
#   v8g  — SOAP >> typo, SearchTransactionsResult check, maxRecordCount 1000
#   v8g-fix  — char-by-char XML write → single f.write(xml_data)
#   v8g-fix2 — stale interval file purge before h loop
#   ClaimSync1a — Phase 0: httpx + ConfigProvider + remove sleep(10)
#
# Deployment: Docker container with httpx_soap.py + config_provider.py
#             in the same folder. shafafiaapi.ini mounted as volume.
# ============================================================================
...
# ClaimSync1a Phase 0: httpx SOAP helpers replace curl subprocess
from httpx_soap import soap_search_transactions, soap_download_transaction
# ClaimSync1a Phase 0: ConfigProvider abstraction — swap to DBConfigProvider in Phase 2
	...
	with open(inifilenamewithfolder, "w") as inif:
		...
		for facindex in range(facilitycnt):
			...
			if os.path.exists(appfolder_path):
				...
				inif.write(mainline)
				# v8g: transactionStatus=2 means "already downloaded" — correct for historical
				# backfill (defaultsearch=n). For daily scheduled sync change to 1 (new/undownloaded).
				...
				inif.write(mainline)
				# v8g: API spec §2.2.4 caps SearchTransactions at 1000 records regardless
				# of requested value. Corrected from 100000 — 2-hour interval splitting
				# in v8b handles the cap; this value is cosmetic only.
				mainline = 'maxRecordCount=1000\n'
				inif.write(mainline)

		# v8g: removed no-op inif.close (missing parentheses) — with block handles closure
	return
...
def FormatDownloadTxnRequestFile(FileID):
	...
	with open(downloadtxnrequestfile, "w") as hrf:
		...
		hrf.write(line007)
		line008 = "      </v2:DownloadTransactionFile>"+ '\n'   # v8g: was ">>" — malformed XML, fixed to single >
		hrf.write(line008)
		line009 = "  </soapenv:Body>"+ '\n'
		hrf.write(line009)
		line010 = "</soapenv:Envelope>"
		hrf.write(line010)
		# v8g: removed no-op hrf.close (missing parentheses) — with block handles closure
...
def DownloadHistoryTxnFile(FileID, fileseqno, transactionID ):
	...
	with open(requestfile, "r") as fin:
		...
		# ClaimSync1a Phase 0: Replace curl subprocess with httpx SOAP call.
		# soap_download_transaction() writes the response to 'responsefile' and returns
		# the path on success, None on failure — same file contract as curl -o had.
		...
		with open(responsefile, "r") as infile, open(responsefile_1, "w") as outfile:
			...
			logline = logwriter('i', 'Dnld:1.8 responsefile converted to single line as '+ responsefile_1)
			dlfh.write(f"{logline}")
	...
	# v8g: removed no-op outfile.close, infile.close (missing parentheses) — with blocks handle closure
	...
	if FileNameXtn.lower() == '.xml':
		...
		try:
			with open(finalfilewithfolder, 'w', encoding='utf-8') as f:
				# v8g-fix: Write entire decoded XML in one call.
				# Previous loop iterated CHARACTERS not lines (~47,000 write()
				# syscalls per file on Windows → multi-minute hang per file).
				f.write(xml_data)
		except OSError as ose:
...
def GetHistoryTxnFileDownload(xmlBody, transactionID):
	...
	# v8g: removed no-op txnfileoutput.close (missing parentheses) — with block handles closure
...
def build_and_execute_search_request(
	...
	# --- Execute HTTP POST via httpx (ClaimSync1a Phase 0: replaces curl subprocess) ---
	# soap_search_transactions() is synchronous/blocking — no shell=True, no
	# subprocess.Popen, explicit timeouts. Returns resp_fname on success, None on failure.
	# The v8g Shafafiya result code check (SearchTransactionsResult) is now inside
	# soap_search_transactions() itself — same contract as before.
...
def mainsub(downloadtype, active, claim_remit):
	...
	# -----------------------------------------------------------------------
	# v8g-fix: Purge stale interval XML files from previous runs.
	# Without this, a previous run with a wider date range leaves higher-indexed
	# response files in Temp (e.g. _12.xml through _35.xml from a 3-day run).
	# The hff glob picks them all up, re-downloading wrong-date files.
	# Purge both request and response files for this facility+claim_remit
	# before writing new ones so hff only ever sees the current run's files.
	# -----------------------------------------------------------------------
	stale_pattern_req  = systemfolder + f"search_history_request_{facility}_{claim_remit}_*.xml"
	stale_pattern_resp = systemfolder + f"search_history_response_{facility}_{claim_remit}_*.xml"
	stale_purge_count  = 0
	for stale_file in glob.glob(stale_pattern_req) + glob.glob(stale_pattern_resp):
		try:
			os.remove(stale_file)
			stale_purge_count += 1
		except OSError as e:
			logline = logwriter('w', f'His:1.10a Could not remove stale file {stale_file}: {e}')
			dlfh.write(f"{logline}")
	if stale_purge_count > 0:
		logline = logwriter('i', f'His:1.10b Purged {stale_purge_count} stale interval XML file(s) from previous runs')
		dlfh.write(f"{logline}")
		print(f"  [{facility}][{claim_remit}] Purged {stale_purge_count} stale interval file(s) from Temp folder")
	...
	for idx, (wstart, wend) in enumerate(intervals):
		...
		# NOT a correctness guard. It is a courtesy rate-limit pause between API calls
		# to the Shafafiya endpoint. Kept at 3s — do not reduce.
		time.sleep(3)
...
def main():
	...
	# v8d: Minimum free disk space required before writing any downloaded file.
	# If free space on the destination drive drops below this threshold the
	# download is skipped cleanly rather than crashing mid-write.
	# Increase if your files are large zip bundles. Value is in megabytes.
		...
		while True:
			...
			try:
				...
				if choice == 9:
					...
					downloadtype = handle_option(choice)
					break
			except ValueError:
	...
	#print("trigger datetime", formatted_datetime)
	#Open log file...
	downloadlogfile = 'donwloadlog-'+formatted_datetime+'.log'
	dlfh = open(downloadlogfile, 'a')
	logline = logwriter('i', 'Pre:1.1 log file: '+downloadlogfile+' Opened Successfully')
	dlfh.write(f"{logline}")

	#logline = logwriter('i', 'Pre:1.4 Valid Client Environment: '+localhostname+' will proceed Successfully')
	#dlfh.write(f"{logline}")

	if len(sys.argv) > 1 or downloadtype == 'newclient' or downloadtype == 'vallic' or downloadtype == 'renlic':
		if len(sys.argv) > 1:
			downloadtype = sys.argv[1]  # Access the first argument
			downloadtype = downloadtype.lower()
		else:
			#print('downloadytpe: ', downloadtype)
			#Option a: downloadTxnFilesv7 a
			if (downloadtype == 'newclient' or downloadtype == 'renlic' or downloadtype == 'vallic'):
				logline = logwriter('i', 'New:0.1 Option : a : Admin Login Initiated')
				dlfh.write(f"{logline}")
				adminvalidated = 'n'
				adminvalidated = askitsolsauthenticate() #commented temporarily...
				if adminvalidated == 'y':
					print("Admin Authentication successful")
					if downloadtype == 'newclient' :
						createini()
				else:
					print("Admin Authentication failed")

		ASKITSolsAPIIni = 'shafafiaapi.ini'
		if os.path.exists(ASKITSolsAPIIni) and downloadtype != 'newclient':
			# ClaimSync1a Phase 0: LocalINIProvider replaces raw configparser.ConfigParser().
			# get_main_config() returns an identical ConfigParser object — all
			# config['section']['key'] accesses below are unchanged.
			# Phase 2 swap: provider = DBConfigProvider(tenant_id) — nothing else changes.
			provider = LocalINIProvider(ASKITSolsAPIIni)
			config = provider.get_main_config()
			logline = logwriter('i', 'Pre:1.5 '+ASKITSolsAPIIni+' found and will proceed to get the Current Configurations')
			dlfh.write(f"{logline}")
			active = config['shafaapi-main']['active'].lower()
			if active != 'y':
				print("App Setup is not (active=y)... Exiting")
				logline = logwriter('w', 'Pre:1.5a '+"App Setup is not (active=y)... Exiting")
				dlfh.write(f"{logline}")
				return
		
			if downloadtype == 'renlic':
				default_lic_enddate = get_end_of_year_date()
				inputvalue = get_user_input("Please input License End Date : ", default_lic_enddate)
				enclicenddate = encryptdecrypt('e', str(inputvalue))
				config = configparser.ConfigParser()
				config.read(ASKITSolsAPIIni)
				config.set('shafaapi-main', 'validuntil',enclicenddate)
				with open(ASKITSolsAPIIni, 'w') as configfile:
					config.write(configfile)
					print('Licence has been Extended Until: ', inputvalue)
					logline = logwriter('w', 'Pre:1.2d '+'Licence has been Extended Until: '+str(inputvalue))
					dlfh.write(f"{logline}")
				# v8g: removed no-op configfile.close (missing parentheses) — with block handles closure
				return

			validuntil = config['shafaapi-main']['validuntil']
			decvaliduntil = encryptdecrypt('d', validuntil)
			if decvaliduntil < str(date.today()):
				print("Licence is Expired, it's valid until:", decvaliduntil, ' Only... Contact Vendor Support!')
				logline = logwriter('w', 'Pre:1.2a '+"Licence is Expired, it's valid until: "+decvaliduntil+' Only... Contact Vendor Support!')
				dlfh.write(f"{logline}")
				return
			else:
				print('Licence is Valid Until: ', decvaliduntil, ' Only...')

			enchost = config['shafaapi-main']['enchost']
			localhostname = platform.node()
			#print('localhostname', localhostname)
			dechost = encryptdecrypt('d', enchost)
			#print('dechost', dechost)

			validclients = enchost
			# check localhost is valid licensed one
			if localhostname ==  dechost:
				print(f'This License is for the Host: {dechost} ')
				print('Pre:1.2b '+localhostname+' is the Valid Host to use the ASKITSols 1.0')
				logline = logwriter('i', 'Pre:1.2b '+localhostname+' is the Valid Host to use the ASKITSols 1.0')
				dlfh.write(f"{logline}")
			else:
				print('\n\n\n')
				print("\033[31m" + "*** Critical Warning ***" + "\033[0m", end="")
				print('\n\n\n')
				print(f'This License is for the Host: {dechost} ')
				print(f'{localhostname} is NOT a Valid Host to use the ASKITSols 1.0 ')
				print("Sorry unable to proceed, Contact System Support...")
				logline = logwriter('c', 'Pre:1.2 '+localhostname+' is NOT the Valid Host to use the ASKITSols 1.0')
				dlfh.write(f"{logline}")
				logline = logwriter('c', 'Pre:1.3 Sorry unable to proceed, Contact System Support... ')
				dlfh.write(f"{logline}")
				return

			if downloadtype == 'vallic':
				return

			#validclients = config['shafaapi-main']['validclients'].lower()
			noofsetup = int(config['shafaapi-main']['noofsetup'])
			systemfolder = config['shafaapi-main']['systemfolder']
			systemfolder = systemfolder.strip('"')
			tempfolder = config['shafaapi-main']['tempfolder']
			tempfolder = tempfolder.strip('"')
			#print("systemfolder: ", systemfolder)
			tempfiles = tempfolder+'*.xml'

			#print("tempfiles", tempfiles)
			logline = logwriter('i', 'Pre:1.6 System related Temp files will be created under: '+systemfolder)
			dlfh.write(f"{logline}")

			#print("going to delete: ", tempfiles)
			logline = logwriter('i', 'Pre:1.7 going to delete: '+tempfiles)
			dlfh.write(f"{logline}")

			files = os.listdir(tempfolder)
			for file in files:
				#os.remove(file) # delete all files
				#if Path(file).suffix.lower() == ".xml" and Path(file).name != "search_history_request.xml" and Path(file).name != "search_history_response.xml":
				if Path(file).suffix.lower() == ".xml" and Path(file).name != "search_history_*.xml":
					os.remove(tempfolder+file) # delete file based on their name or suffix
					logline = logwriter('i', 'Pre:1.7a '+file + ' Got deleted Successfully...')
					dlfh.write(f"{logline}")
			#print("To Download New Txn files 2")
			if active == "y":
				#print("To Download New Txn files 3")
				#print ("proceed to the function")
				try:
					for currentsetup in range(noofsetup):
						currentsetup = currentsetup + 1
						#print("Current setup to process: ", currentsetup)
						userid = config['client-config-'+str(currentsetup)]['userid']
						password = config['client-config-'+str(currentsetup)]['password']
						facility = config['client-config-'+str(currentsetup)]['facility']
						claims = config['client-config-'+str(currentsetup)]['claims']
						resubmission = config['client-config-'+str(currentsetup)]['resubmission']
						remittance = config['client-config-'+str(currentsetup)]['remittance']
						#print("Setup: ", currentsetup, "User id:"+userid)
						#print("Setup: ", currentsetup, "password:"+password)
						#print("Setup: ", currentsetup, "facility:"+facility)
						#print("Setup: ", currentsetup, "dnloadfolder:"+dnload_remittance_folder)
						logline = logwriter('i', 'Pre:1.10 Required userid, password and facility retrieved from INI')
						dlfh.write(f"{logline}")
						print("Processing Setup: (h-claim)", currentsetup)
						logline = logwriter('i', "Processing Setup: (h-claim): "+str(currentsetup))
						dlfh.write(f"{logline}")
						mainsub('h', active, 'claim')
						# ClaimSync1a: sleep(10) removed — httpx is synchronous/blocking, no curl race condition
						print("Processing Setup: (hf-claim)", currentsetup)
						logline = logwriter('i', "Processing Setup: (hf-claim) "+str(currentsetup))
						dlfh.write(f"{logline}")
						mainsub('hf', active, 'claim')
						# ClaimSync1a: sleep(10) removed — httpx is synchronous/blocking, no curl race condition
						print("Processing Setup: (hff-claim)", currentsetup)
						logline = logwriter('i', "Processing Setup: (hff-claim) "+str(currentsetup))
						dlfh.write(f"{logline}")
						mainsub('hff', active, 'claim')
						print("Processing Setup: (h-remit)", currentsetup)
						logline = logwriter('i', "Processing Setup: (h-remit) "+str(currentsetup))
						dlfh.write(f"{logline}")
						mainsub('h', active, 'remit')
						# ClaimSync1a: sleep(10) removed — httpx is synchronous/blocking, no curl race condition
						print("Processing Setup: (hf-remit)", currentsetup)
						logline = logwriter('i', "Processing Setup: (hf-remit)"+str(currentsetup))
						dlfh.write(f"{logline}")
						mainsub('hf', active, 'remit')
						# ClaimSync1a: sleep(10) removed — httpx is synchronous/blocking, no curl race condition
						print("Processing Setup: (hff-remit)", currentsetup)
						logline = logwriter('i', "Processing Setup: (hff-remit)"+str(currentsetup))
						dlfh.write(f"{logline}")
						mainsub('hff', active, 'remit')
				except OSError as ose:
					# v8d: Top-level catch for disk-full OSError that was not caught lower down.
					# Any unexpected errno 28 that escapes GetHistoryTxnFileDownload lands here.
					# Log CRITICAL and exit cleanly — no raw crash traceback shown to user.
					if ose.errno == errno.ENOSPC:
						logline = logwriter('c', f'Main:DISK-FULL OSError errno 28 in facility loop — disk is full. Exiting cleanly.')
						dlfh.write(f"{logline}")
						print('\n\n')
						print("\033[31m" + "*** DISK FULL — No space left on device ***" + "\033[0m")
						print('The destination drive ran out of space during file download.')
						print('Please free up disk space, then re-run. Already downloaded files are safe.')
						print('(Idempotent design: re-run will skip files already on disk and resume from where it stopped.)')
					else:
						logline = logwriter('c', f'Main:OSError [{ose.errno}] in facility loop: {ose}')
						dlfh.write(f"{logline}")
						print(f'\n  *** Unexpected OSError [{ose.errno}]: {ose}')
						raise
			else:
				print ("ASKITAPISols 1.0 not Activated")
				logline = logwriter('w', 'Warn:1.1 ASKITAPISols 1.0 not Activated')
				dlfh.write(f"{logline}")
				return
			#print("To Download New Txn files 4")
		elif downloadtype != 'newclient':
			print('\n\n\n')
			print("\033[31m" + "*** Critical Warning ***" + "\033[0m", end="")
			print("\nASKITAPISols 1.0 key configuration file: 'shafafiaapi.ini' Not Found")
			print("Sorry unable to proceed, Contact System Support...")

			logline = logwriter('c', 'Crit:1.1 *** Critical Warning ***')
			dlfh.write(f"{logline}")
			logline = logwriter('c', "Crit:1.2 ASKITAPISols 1.0 key configuration file: 'shafafiaapi.ini' Not Found")
			dlfh.write(f"{logline}")
			logline = logwriter('c', 'Crit:1.3 Sorry unable to proceed, Contact System Support...')
			dlfh.write(f"{logline}")
			return
	else:
		print("Valid Input parameter should be either n (new txn files) or h (history files)")
		logline = logwriter('i', 'Pre:1.11 Valid Input parameter should be either n (new txn files) or h (history files)')
		dlfh.write(f"{logline}")
