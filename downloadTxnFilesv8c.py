...
import sys, configparser, subprocess, os, time, platform, re
...
def DownloadHistoryTxnFile(FileID, fileseqno, transactionID ):
	...
	if FileNameXtn.lower() == '.xml':
		...
		if transactionID == '2':
			...
			#added on 25th May to remove the attachments from resubmission files
	...
	#print('finalfile to print: ', finalfile)
	return finalfile
...
def GetHistoryTxnFileDownload(xmlBody, transactionID):
	...
	with open(downloadedfileslog, 'w') as txnfileoutput:
		...
		while nextFileNameloc > 0:
			...
			# v8c: DownloadHistoryTxnFile returns '' when Shafafiya API gave a
			# non-zero result code (e.g. -6 file not found).  Skip the disk check
			# in that case — there is no file to look for and no action to take.
			if downloadedFilename == '':
				...
				count_downloaded += 1
				#print("File: "+downloadedFilename +" Downloaded Successfully")
				logline = logwriter('i', "HisDnld:1.5 File: "+downloadedFilename +" Downloaded Successfully")
				dlfh.write(f"{logline}")
			else:
				count_notfound += 1
				#print("Looks File: "+downloadedFilename +" Not Downloaded Successfully, please recheck")
				logline = logwriter('i', "HisDnld:1.6 Looks File: "+downloadedFilename +" Not Downloaded Successfully, please recheck")
				dlfh.write(f"{logline}")
...
def mainsub(downloadtype, active, claim_remit):
	...
	for idx, (wstart, wend) in enumerate(intervals):
		...
		if result:
			...
			dlfh.write(f"{logline}")
		# Small pause between API calls to be polite to the Shafafiya endpoint
		time.sleep(3)
