...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    success: 'bg-emerald-100 text-emerald-700', running: 'bg-amber-100 text-amber-700',
    failed: 'bg-red-100 text-red-700', partial: 'bg-yellow-100 text-yellow-700',
  }
              ...
              {runs.map(run => (
                <div key={run.run_id}>
                  <div className="grid grid-cols-6 gap-3 px-5 py-3 items-center hover:bg-gray-50 transition-colors">
                    ...
                    <div><StatusBadge status={run.status} />{run.trigger_type === 'manual' && <span className="ml-1 text-xs text-blue-500">(adhoc)</span>}</div>
