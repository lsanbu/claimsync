...
interface RunRecord {
  ...
  trigger_type:         string
  // v2.9 live counters — API computes these from file_manifest /
  // sync_run_intervals child tables since the engine doesn't update
  // sync_run_log mid-run. Use these while status='running'.
  live_files_count:     number
  live_intervals_count: number
  // v2.25 — engine :3.17+: currently-processing interval window (cleared at end_run)
  current_interval_from: string | null
  current_interval_to:   string | null
}
...
// v2.25: engine writes the currently-processing window as 'YYYY-MM-DD HH:MM'.
// That string isn't strictly ISO, so .replace(' ', 'T') is needed before
// new Date() — otherwise Safari and some Node versions return Invalid Date.
...
export default function AdhocRunPage() {
  ...
  // Only runs while a row is in 'running' state so it idles to nothing afterwards.
              ...
              {runs.map(run => (
                <div key={run.run_id}>
                  ...
                  {/* v2.24: live progress strip — only when status='running'.
                      Spans full row width so the bar + ETA have room to breathe.
                      Denominator is intervals_total (sync_run_log column written
                      by update_progress in engine :3.16), NOT live_intervals_count
                      — they tick up together so the ratio would always be ≈100%. */}
                  {run.status === 'running' && (() => {
