"""
routers/stats.py — /stats endpoints
--------------------------------------
Column mapping (sync_run_log actual schema):
  files_duplicate  → files_resubmission
  files_skipped    → files_skipped_existing + files_skipped_api_error
  duration_seconds → GENERATED ALWAYS column (read directly)
"""
...
from ..models import RunStats, PayerStat

router = APIRouter()


@router.get("/summary", response_model=RunStats, summary="Overall run statistics")
def run_summary(
    ...
    fac_subfilter = ("AND r2.facility_id = %s::uuid" if facility_id else "")
    ...
    return RunStats(**(row or {}))


@router.get("/payers", response_model=list[PayerStat], summary="Files per payer")
def payer_breakdown(
    facility_id: Optional[str] = Query(None),
    days:        int            = Query(30, ge=1, le=365),
):
    ...
    rows = query(
        f"""
        SELECT
            COALESCE(f.payer, 'Unknown') AS payer,
            COUNT(*)                      AS file_count,
    ...
    return [PayerStat(**r) for r in rows]
