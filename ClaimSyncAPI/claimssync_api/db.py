...
def _run_migrations(conn):
    """Apply idempotent schema migrations on connect."""
    try:
        with conn.cursor() as cur:
            # v3.8: Add Shafafiya metadata columns to file_manifest
            cur.execute(f"""
                ALTER TABLE {SCHEMA}.file_manifest
        ...
        logger.warning(f"DB migration warning (non-fatal): {exc}")
